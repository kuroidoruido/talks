...
export default function scenario2() {
    const api = createTodolistClient('http://localhost:8080', 'scenario2');
    ...
    waitRealisticDelay();
    const deletedFirstTodo = api.deleteOneTodoItem(firstTodo.id);
    check(deletedFirstTodo, {
        '[delete first item] deleting first todo is ok': checkNotAnErrorResponse,
        '[delete first item] the returned todo is ok': (res) => 
            res !== null && res !== undefined && res.status === firstTodo.status && res.id === firstTodo.id && res.description === firstTodo.description,
    });
    waitRealisticDelay();
    todos = api.getAllTodoItems();
    check(todos, {
        '[after delete first item] getting all todo is ok': checkNotAnErrorResponse,
        '[after delete first item] fist todo is updated': (res) => res.length === 0,
    });
}