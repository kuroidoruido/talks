...
<!--
- on peut faire exécuter plusieurs langages sur la JVM : JavaScript (dont Node.js), Ruby, Python, R, LLVM et WebAssembly
...
+            Value fn = context.eval("js", "function foo(x) { return x+y+z; } foo"});
            System.out.println(fn.execute(x)); // will print NaN
        }
    }
}
```
...
- un ingénieur Twitter disait y'a 2-3 ans disait : chaque tweet passe au moins par un morceau de code qui tourne avec GraalVM
- Des frameworks ont été construits spécialement pour être efficace avec GraalVM en natif : Micronaut ou Quarkus
...
- la consommation mémoire est vraiment plus faible
-->
